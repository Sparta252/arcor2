...
class AbstractDobot(FitCommonMixin, Robot):
    ...
    def my_pickup_moving_object(self, starting_pose: Pose, belt_pose: Pose, belt_speed: float = 5.0, direction: Direction = Direction.LEFT, *, an: str | None = None) -> Pose:
        ...
        :param direction: reserved for future use; belt direction is computed from belt_pose orientation
        """
        return rest.call(
            rest.Method.POST,
            f"{self.settings.url}/pickup/moving_object",
            body=[starting_pose, belt_pose],
            params={"belt_speed": belt_speed},
            return_type=Pose,
        )
